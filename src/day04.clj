...
(defn parse-card [l]
  (->> (str/split l #"\s*[:+\|]\s*")
       next
       (mapv #(into #{} (map parse-long) (str/split % #"\s+")))))

(defn count-winning [[win-cards cards]]
  (count (set/intersection win-cards cards)))
